...
import "@openzeppelin/contracts/access/Ownable.sol";
    ...
    interface IManagment{
        function linkBatteriestoLot(uint256, uint256[3] memory) external;
    }

contract BatteriesLOT is ERC721URIStorage, Ownable{
    uint public tokenCount;
    IManagment public Management;
    constructor () ERC721("Batteries LOT", "BTRLOT"){
    ...
    function mint(string memory _tokenURI, uint256[3] memory _childIDs, address _LotCreator) external onlyManagementSmartContract returns(uint, string memory) {
        ...
        Management.linkBatteriestoLot(tokenCount, _childIDs);
