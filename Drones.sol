...
import "@openzeppelin/contracts/access/Ownable.sol";
    ...
    interface IManagment{
        function linkComponentstoDrone(uint256, uint256[4] memory) external;
    }

contract Drone is ERC721URIStorage, Ownable{
    ...
    function mint(string memory _tokenURI, address _LotCreator) external onlyManagementSmartContract returns(uint, string memory) {
        tokenCount++;
        _safeMint(_LotCreator, tokenCount); 
        _setTokenURI(tokenCount, _tokenURI);
        //Management.linkComponentstoDrone(tokenCount, _childIDs);
